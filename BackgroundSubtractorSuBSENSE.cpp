...
void BackgroundSubtractorSuBSENSE::initialize(const cv::Mat& oInitImg, const cv::Mat& oROI) {
	...
	if(oInitImg.type()==CV_8UC3) {
		...
		voInitImgChannels.resize(3);*/
		cv::Mat voInitImgChannels[3];
	...
	if(oROI.empty() && (m_oROI.empty() || oROI.size()!=oInitImg.size())) {
		...
		oNewBGROI = m_oROI;//m_oROI : background model ROI used for LBSP descriptor extraction (specific to the input image size);
	else {
		...
		cv::Mat oTempROI;
		cv::dilate(oNewBGROI,oTempROI,cv::Mat(),cv::Point(-1,-1),LBSP::PATCH_SIZE/2);//dilate 
	...
	m_oUpdateRateFrame.create(m_oImgSize,CV_32FC1);  //T(x);
	m_oUpdateRateFrame = cv::Scalar(m_fCurrLearningRateLowerCap);
	m_oDistThresholdFrame.create(m_oImgSize,CV_32FC1);
	m_oDistThresholdFrame = cv::Scalar(1.0f); // R(x);
	m_oVariationModulatorFrame.create(m_oImgSize,CV_32FC1);//relative value used to modulate 'R(x)' and 'T(x)' variations;
	m_oVariationModulatorFrame = cv::Scalar(10.0f); // should always be >= FEEDBACK_V_DECR;
	m_oMeanLastDistFrame.create(m_oImgSize,CV_32FC1); //'D_last(x)', used to detect ghosts and high variation regions in the sequence;
	m_oMeanLastDistFrame = cv::Scalar(0.0f);
	m_oMeanMinDistFrame_LT.create(m_oImgSize,CV_32FC1);//per-pixel mean minimal distances from the model ('D_min(x)' in PBAS, used to control variation magnitude and direction of 'T(x)' and 'R(x)');
	...
	m_oUnstableRegionMask.create(m_oImgSize,CV_8UC1);//a lookup map used to keep track of unstable regions (based on segm. noise & local dist. thresholds)
	m_oUnstableRegionMask = cv::Scalar_<uchar>(0);
	m_oBlinksFrame.create(m_oImgSize,CV_8UC1);// per-pixel blink detection map ('Z(x)')
	m_oBlinksFrame = cv::Scalar_<uchar>(0);
	m_oDownSampledFrame_MotionAnalysis.create(m_oDownSampledFrameSize,CV_8UC((int)m_nImgChannels));// pre-allocated matrix used to downsample the input frame when needed
	m_oDownSampledFrame_MotionAnalysis = cv::Scalar_<uchar>::all(0);
	m_oLastColorFrame.create(m_oImgSize,CV_8UC((int)m_nImgChannels));

	m_oLastColorFrame = cv::Scalar_<uchar>::all(0);//copy of latest pixel intensities (used when refreshing model)
	m_oLastDescFrame.create(m_oImgSize,CV_16UC((int)m_nImgChannels));
	m_oLastDescFrame = cv::Scalar_<ushort>::all(0);//copy of latest descriptor
	m_oLastRawFGMask.create(m_oImgSize,CV_8UC1);
	m_oLastRawFGMask = cv::Scalar_<uchar>(0);//the foreground mask generated by the method at [t-1] (without post-proc, used for blinking px detection)
	m_oLastFGMask.create(m_oImgSize,CV_8UC1); 
	m_oLastFGMask = cv::Scalar_<uchar>(0);// the foreground mask generated by the method at [t-1]
	...
	effectPix.create(1,m_nBGSamples,CV_8UC1);  
	effectPix = cv::Scalar_<uchar>(0);


	m_voBGColorSamples.resize(m_nBGSamples);  //background model pixel color intensity samples (equivalent to 'B(x)' in PBAS)
	m_voBGDescSamples.resize(m_nBGSamples);//(m_nBGSamples=N);

	for(size_t s=0; s<m_nBGSamples; ++s) {
		m_voBGColorSamples[s].create(m_oImgSize,CV_8UC(((int)m_nImgChannels)+1));
	...
	if(m_aPxInfoLUT)
	    delete[] m_aPxInfoLUT;
	m_aPxIdxLUT = new size_t[m_nTotRelevantPxCount];//total number of relevant pixels
	m_aPxInfoLUT = new PxInfoBase[m_nTotPxCount]; //total number of pixels (depends on the input frame size) 
	 ...
	 //m_nImgChannels==3
		...
		for(size_t t=0; t<=UCHAR_MAX; ++t) //  UCHAR_MAX=255;
			m_anLBSPThreshold_8bitLUT[t] = cv::saturate_cast<uchar>(m_nLBSPThresholdOffset+t*m_fRelLBSPThreshold); 
		//m_nLBSPThresholdOffset:    LBSP internal threshold offset value, used to reduce texture noise in dark regions
		// m_fRelLBSPThreshold:    LBSP relative internal threshold (kept here since we don't keep an LBSP object)
		//m_anLBSPThreshold_8bitLUT:  previous-allocated internal LBSP threshold values LUT for all possible 8-bit intensities
		...
		for(size_t nPxIter=0, nModelIter=0; nPxIter<m_nTotPxCount; ++nPxIter) {
			if(m_oROI.data[nPxIter]) {
				...
				const size_t nPxRGBIter_1 = nPxIter*4;
				const size_t nDescRGBIter = nPxRGBIter*2;

				for(size_t c=0; c<3; ++c) {
					m_oLastColorFrame.data[nPxRGBIter+c] = oInitImg.data[nPxRGBIter+c];//copy the last frame pixel intensity used for refresh;
	...
	m_bInitialized = true; //specifies whether the algorithm is fully initialized or not
...
void BackgroundSubtractorSuBSENSE::refreshModel(float fSamplesRefreshFrac, bool bForceFGUpdate) {
	// == refresh  default bForceFGUpdate=false;
	...
	//nModelsToRefresh=N,i.e. m_nBGSamples
	...
	//nRefreshStartPos=0
	 ...
	 //m_nImgChannels==3
		for(size_t nModelIter=0; nModelIter<m_nTotRelevantPxCount; ++nModelIter) {
			...
			const size_t nPxIter = m_aPxIdxLUT[nModelIter];////total number of relevant pixels
			...
			if(bForceFGUpdate || !m_oLastFGMask.data[nPxIter]) {
				...
				for(size_t nCurrModelIdx=nRefreshStartPos; nCurrModelIdx<nRefreshStartPos+nModelsToRefresh; ++nCurrModelIdx) {
					...
					if(bForceFGUpdate || !m_oLastFGMask.data[nSamplePxIdx]) {
						...
						for(size_t c=0; c<3; ++c) {
							m_voBGColorSamples[nCurrRealModelIdx].data[nPxIter*4+c] = m_oLastColorFrame.data[nSamplePxIdx*3+c];

							*((ushort*)(m_voBGDescSamples[nCurrRealModelIdx].data+(nPxIter*3+c)*2)) = *((ushort*)(m_oLastDescFrame.data+(nSamplePxIdx*3+c)*2));
						}
						m_voBGColorSamples[nCurrRealModelIdx].data[nPxIter*4+3] = 1;

						for (size_t ep=0; ep<5; ++ep) {
							const size_t s_rand = rand()%m_nBGSamples;
							m_voBGColorSamples[s_rand].data[nPxIter*4+3] += 1;
...
void BackgroundSubtractorSuBSENSE::operator()(cv::InputArray _image, cv::OutputArray _fgmask, double learningRateOverride) {
	// == process   default(learningRateOverride=0)
	CV_Assert(m_bInitialized);
	...
	const float fRollAvgFactor_ST = 1.0f/std::min(m_nFrameIndex,m_nSamplesForMovingAvgs/4);
	 //m_nImgChannels==3
		for(size_t nModelIter=0; nModelIter<m_nTotRelevantPxCount; ++nModelIter) {
			...
			const size_t nPxIterRGB_1 = nPxIter*4;
			...
			size_t nMinTotDescDist=s_nDescMaxDataRange_3ch;//48U    
			size_t nMinTotSumDist=s_nColorMaxDataRange_3ch;//255U
			float* pfCurrDistThresholdFactor = (float*)(m_oDistThresholdFrame.data+nFloatIter);// m_oDistThresholdFrame: R(x)  
			float* pfCurrVariationFactor = (float*)(m_oVariationModulatorFrame.data+nFloatIter); //per-pixel distance variation modulators ('v(x)', relative value used to modulate 'R(x)' and 'T(x)' variations)
			float* pfCurrLearningRate = ((float*)(m_oUpdateRateFrame.data+nFloatIter));// T(x)
			...
			m_oUnstableRegionMask.data[nPxIter] = ((*pfCurrDistThresholdFactor)>UNSTABLE_REG_RDIST_MIN || (*pfCurrMeanRawSegmRes_LT-*pfCurrMeanFinalSegmRes_LT)>UNSTABLE_REG_RATIO_MIN || (*pfCurrMeanRawSegmRes_ST-*pfCurrMeanFinalSegmRes_ST)>UNSTABLE_REG_RATIO_MIN)?1:0;//统计不稳定区域;
			...
			while( goodDist<m_nRequiredBGSamples && nSampleIdx<m_nBGSamples) {
				...
				const uchar* const anBGColor = m_voBGColorSamples[nSampleIdx].data+nPxIterRGB_1;
				effectPix.data[nSampleIdx] = 0;
				...
				goodDist += anBGColor[3];  
				effectPix.data[nSampleIdx] = 1; 
				sumeffectPix += effectPix.data[nSampleIdx]; 
			...
			*pfCurrMeanLastDist = (*pfCurrMeanLastDist)*(1.0f-fRollAvgFactor_ST) + fNormalizedLastDist*fRollAvgFactor_ST;
			...
			if(goodDist < m_nRequiredBGSamples) {
				// == foreground
				const float fNormalizedMinDist = std::min(1.0f,((float)nMinTotSumDist/s_nColorMaxDataRange_3ch+(float)nMinTotDescDist/s_nDescMaxDataRange_3ch)/2 + (float)(m_nRequiredBGSamples-goodDist)/m_nRequiredBGSamples);
				...
				*pfCurrMeanRawSegmRes_ST = (*pfCurrMeanRawSegmRes_ST)*(1.0f-fRollAvgFactor_ST) + fRollAvgFactor_ST;
				...
				if(m_nModelResetCooldown && (rand()%(size_t)FEEDBACK_T_LOWER)==0) {
					...
					for(size_t c=0; c<3; ++c) {
						...
						*(m_voBGColorSamples[s_rand].data+nPxIterRGB_1+c) = anCurrColor[c];
					}
				...
				*pfCurrMeanRawSegmRes_ST = (*pfCurrMeanRawSegmRes_ST)*(1.0f-fRollAvgFactor_ST);
				...
				if((rand()%nLearningRate)==0) {	
					...
					for(size_t ms=0; ms < m_nBGSamples; ++ms) {	
						if (effectPix.data[ms] == 1) {
							*(m_voBGColorSamples[ms].data+nPxIterRGB_1+3) += 1/sumeffectPix;
						} 
						else {
							*(m_voBGColorSamples[ms].data+nPxIterRGB_1+3) -= 1/(m_nBGSamples-sumeffectPix);
							if (*(m_voBGColorSamples[ms].data+nPxIterRGB_1+3) < 0)    *(m_voBGColorSamples[ms].data+nPxIterRGB_1+3) = 0;
						}
					}


					size_t mt=0;
					size_t weightmax = *(m_voBGColorSamples[0].data+nPxIterRGB_1+3);
					size_t weightmin = *(m_voBGColorSamples[0].data+nPxIterRGB_1+3);
					size_t weightmaxdot=0, weightmindot=0; 
					while( mt < m_nBGSamples ) {
						if ( weightmax < (*(m_voBGColorSamples[mt].data+nPxIterRGB_1+3))){
						weightmax=(*(m_voBGColorSamples[mt].data+nPxIterRGB_1+3));
						weightmaxdot = mt;
						}
						if ( weightmin > (*(m_voBGColorSamples[mt].data+nPxIterRGB_1+3))){
						weightmin=(*(m_voBGColorSamples[mt].data+nPxIterRGB_1+3));
						weightmindot = mt;
						}
						++mt;
					}  
					//const size_t s_rand = rand()%m_nBGSamples;
					for(size_t c=0; c<3; ++c) {
						*((ushort*)(m_voBGDescSamples[weightmindot].data+nDescIterRGB+2*c)) = anCurrIntraDesc[c];
						*(m_voBGColorSamples[weightmindot].data+nPxIterRGB_1+c) = anCurrColor[c];
					}
					*(m_voBGColorSamples[weightmindot].data+nPxIterRGB_1+3) += 1;
					size_t count_weight=0;
					for (size_t m_y=0; m_y < m_nBGSamples; ++m_y) {
						if ((*(m_voBGColorSamples[m_y].data+nPxIterRGB_1+3)) > (weightCoe*weightmax)) ++count_weight;
					}

					
					for (size_t m_z=0; m_z < m_nBGSamples; ++m_z) {
						if ((*(m_voBGColorSamples[m_z].data+nPxIterRGB_1+3)) > (weightCoe*weightmax)) {

							*(m_voBGColorSamples[m_z].data+nPxIterRGB_1+3) -= 1/count_weight;
						}
					}
					size_t countTotalWeight=0;
					for (size_t m_s=0; m_s < m_nBGSamples; ++m_s) {
						 countTotalWeight += *(m_voBGColorSamples[m_s].data+nPxIterRGB_1+3);
					}
					if (countTotalWeight != (m_nBGSamples+5) ){
                  *(m_voBGColorSamples[weightmaxdot].data+nPxIterRGB_1+3) += (m_nBGSamples+5-countTotalWeight);
				...
				if((n_rand%(bCurrUsing3x3Spread?nLearningRate:(nLearningRate/2+1)))==0
					...
					const size_t idx_rand_uchar_rgb_1 = idx_rand_uchar*4;
					const size_t idx_rand_ushrt_rgb = idx_rand_uchar_rgb*2;//	const size_t s_rand = rand()%m_nBGSamples;
					size_t mt=0;
					size_t weightmax = *(m_voBGColorSamples[0].data+idx_rand_uchar_rgb_1+3);
					size_t weightmin = *(m_voBGColorSamples[0].data+idx_rand_uchar_rgb_1+3);
					size_t weightmaxdot=0, weightmindot=0; 
					while( mt < m_nBGSamples ) {
						if ( weightmax < (*(m_voBGColorSamples[mt].data+idx_rand_uchar_rgb_1+3))){
							weightmax=(*(m_voBGColorSamples[mt].data+idx_rand_uchar_rgb_1+3));
							weightmaxdot = mt;
						}
						if ( weightmin > (*(m_voBGColorSamples[mt].data+idx_rand_uchar_rgb_1+3))){
							weightmin=(*(m_voBGColorSamples[mt].data+idx_rand_uchar_rgb_1+3));
							weightmindot = mt;
						}
						++mt;
					}  
					for(size_t c=0; c<3; ++c) {
						*((ushort*)(m_voBGDescSamples[weightmindot].data+idx_rand_ushrt_rgb+2*c)) = anCurrIntraDesc[c];

						*(m_voBGColorSamples[weightmindot].data+idx_rand_uchar_rgb_1+c) = anCurrColor[c];
					}
					*(m_voBGColorSamples[weightmindot].data+idx_rand_uchar_rgb_1+3) +=1;
					//const size_t s_rand = rand()%m_nBGSamples;
					size_t count_weight=0;
					for (size_t m_y=0; m_y < m_nBGSamples; ++m_y) {
						if ((*(m_voBGColorSamples[m_y].data+idx_rand_uchar_rgb_1+3)) > (weightCoe*weightmax)) ++count_weight;
					}

				//	count_weight=m_nBGSamples; 

					for (size_t m_z=0; m_z < m_nBGSamples; ++m_z) {
						if ((*(m_voBGColorSamples[m_z].data+idx_rand_uchar_rgb_1+3)) > (weightCoe*weightmax)) {

							*(m_voBGColorSamples[m_z].data+idx_rand_uchar_rgb_1+3) -= 1/count_weight;
						}
					}
					size_t countTotalWeight=0;
					for (size_t m_s=0; m_s < m_nBGSamples; ++m_s) {
						countTotalWeight += *(m_voBGColorSamples[m_s].data+idx_rand_uchar_rgb_1+3);
					}
					if (countTotalWeight != (m_nBGSamples+5) ){
						*(m_voBGColorSamples[weightmaxdot].data+idx_rand_uchar_rgb_1+3) += (m_nBGSamples+5-countTotalWeight);
					} 
				}
			}
			...
			if((*pfCurrDistThresholdFactor)<std::pow(1.0f+std::min(*pfCurrMeanMinDist_LT,*pfCurrMeanMinDist_ST)*2,2)) //************把2修改称1.5;
